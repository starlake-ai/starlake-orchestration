...
- **Tables loaded by non-scheduled DAGs** (a manual, one-shot, bootstrap or replay load) no longer make a dataset-triggered transform end green having executed nothing — since 0.6.18 (issue #150), the `start` task no longer correlates them on the producing run's data interval, which such a run does not have on Airflow 3:
  - a triggering event whose dataset declares **no cron** is retained by its presence, with no lookup — whatever `data_cycle` says;
  - a cron-less dependency that did **not** trigger the run is found by **publication time**: its most recently published event after the previous successful run's trigger time, up to this run's trigger time (`run_after` on Airflow 3, the logical date on Airflow 2). Neither bound moves when a run is cleared, so re-executing `start` reproduces its verdict. `freshness` no longer caps such an event's `sl_scheduled_date`, and an event without `sl_scheduled_date` counts as present (a readable one still raises the `sl_data_interval_end` pushed by `start`).
  - datasets produced by **scheduled** DAGs keep today's check (issue #151), and a run that fails its check is still skipped silently.
