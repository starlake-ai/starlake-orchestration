...
- Same non-mutating principle for the captured `assets` list (issue #115): each attempt materializes from a per-attempt copy, so retries never yield duplicate `AssetMaterialization`s
