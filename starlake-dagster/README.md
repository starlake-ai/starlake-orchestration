...
| **dataproc_job_wait_timeout**     | int  | (Dataproc only) timeout in seconds to wait for a submitted job to reach its terminal state (`3600` by default) — the submission response is never terminal, so the op polls `wait_for_job`/`get_job`; ignored in sensor mode (each poke's wait is bounded by `pre_load_timeout`); on poll timeout the still-running job is ABANDONED (no cancel primitive) — size it above your longest job |
| **pre_load_sensor**               | bool | `true`/`false` (default `false`) — wrap the pre-load op in an in-op poke loop that re-runs the preload until files arrive, on EVERY execution environment. CAUTION: Dagster has no reschedule primitive, the op HOLDS ITS EXECUTOR SLOT while poking (up to `pre_load_timeout` seconds). On the cloud variants (cloud_run, dataproc, fargate) each poke RE-SUBMITS the cloud job — full submission overhead per attempt (the slot-holding is lightweight there: the heavy work runs cloud-side between checks), and a genuinely broken invocation (bad config, infra failure) is indistinguishable from "no files yet" and pokes until timeout |
