...
class StarlakeDependency(DependencyMixin):
    def __init__(self, name: str, dependency_type: StarlakeDependencyType, cron: Optional[str]= None, dependencies: List[StarlakeDependency]= [], sink: Optional[str]= None, stream: Optional[str]= None, freshness: int = 0, **kwargs):
        ...
        super().__init__(name)
        self._name = name
        # Guard the literal "None" sentinel the same way cron does below:
        # dependency payloads generated by starlake core <= 1.5.15 carry
        # `"sink" : "None"` on table dependencies (the schedule placeholder
        # leaked into the sink field, core issue #1654). Treating that string
        # as a real sink yielded domain="None"/table="None" and the dataset
        # URI "none_none", which can never match the producer's outlet — the
        # dataset-triggered DAG silently never fired (issue #132). Fall back
        # to `name`, which carries the real domain.table in these payloads.
        if sink and sink.strip().lower() != 'none':
